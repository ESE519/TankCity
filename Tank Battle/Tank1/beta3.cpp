...
AnalogIn AinLight1(p17);
AnalogIn AinLight2(p18);
AnalogIn AinLight3(p19);
AnalogIn AinLight4(p20);
DigitalOut Life1(p6);
DigitalOut Life2(p7);
float ADCLight1;
float ADCLight2;
...
void XmoveToOne()
{
	...
	Locate(posoh);
		   ...
		   if(okx1==0)
			...
			if(posoh[2]<1.2 && okx1==1)
			{
				tank.left(0.1);
			}
			else if(posoh[2]>=1.2  &&okx1==1)
			{
				 tank.stop();
			}
